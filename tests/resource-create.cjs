...
f=fixture({failCall:2});f.db._grist_Tables_column=f.db._grist_Tables_column.filter(c=>c.colId!=='Privacy_Safety_Considerations');await f.run('promoteApprovedResource()');assert.equal(f.db.Resource_Submissions[0].Promotion_Result,'Partial');f.db.Resources[1].Purchase_Model='[]';await f.run('recoverPartialCreation()');assert.equal(f.db.Resource_Submissions[0].Promotion_Result,'Success');assert.equal(f.db.Resources[1].Purchase_Model,'');assert(!('Privacy_Safety_Considerations' in f.db.Resources[1]));
f=fixture({failCall:2});await f.run('promoteApprovedResource()');f.db._grist_Tables_column=f.db._grist_Tables_column.filter(c=>c.colId!=='Privacy_Safety_Considerations');await f.run('recoverPartialCreation()');assert.equal(f.db.Resource_Submissions[0].Promotion_Result,'Partial');
f=fixture();f.db._grist_Tables_column=f.db._grist_Tables_column.filter(c=>c.colId!=='Privacy_Safety_Considerations');const draft=JSON.parse(f.db.Resource_Submissions[0].Resource_Review_Draft);draft.fields.Privacy_Safety_Considerations='Must preserve';f.db.Resource_Submissions[0].Resource_Review_Draft=JSON.stringify(draft);await f.run('promoteApprovedResource()');assert.equal(f.db.Resources.length,1);
console.log('PASS: omitted blank fields and missing previously written fields;  schema types, choices, blank values, partial recovery, recovery repeat and external-change protection;  create, exact audit/link, ID allocation and formula ID, types, cancel, repeat/double click, duplicates at confirmation, approval revocation, legacy draft, linked resource, invalid data, first/final write failures, and lost responses.');
