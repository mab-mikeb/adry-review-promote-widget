...
const nodes=new Map();function node(){return {textContent:'',value:'',hidden:false,disabled:false,dataset:{},children:[],classList:{toggle(){},add(){},remove(){}},style:{},appendChild(n){this.children.push(n)},replaceChildren(){this.children=[]},addEventListener(){},setAttribute(){}}}
...
db._grist_Tables_column=['Resource_ID',...defs].map((colId,i)=>({id:i+1,parentId:10,colId,isFormula:colId==='Resource_ID'&&!!options.formulaId,formula:colId==='Resource_ID'&&options.formulaId?'generated':''}));
ctx.source=source;ctx.fields=fields;run("currentRecord=source;currentResource=null;window.__resourceProvidersById=new Map([[1,{id:1,Provider_Name:'AngelSense'}]]);setControl('resourceAction','Create');FIELD_DEFS.forEach(d=>setControl('r_final_'+d.key,fields[d.key]));isDirty=false;");
...
console.log('PASS: create, exact audit/link, ID allocation and formula ID, types, cancel, repeat/double click, duplicates at confirmation, approval revocation, legacy draft, linked resource, invalid data, first/final write failures, and lost responses.');
